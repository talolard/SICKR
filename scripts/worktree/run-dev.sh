...
export DATABASE_URL MILVUS_URI ARTIFACT_ROOT_DIR FEEDBACK_ROOT_DIR TRACE_ROOT_DIR
...
if [[ "${MODE}" == "backend" ]]; then
  exec make chat PORT="${BACKEND_PORT}" DATABASE_URL="${DATABASE_URL}" MILVUS_URI="${MILVUS_URI}"
fi
...
make chat PORT="${BACKEND_PORT}" DATABASE_URL="${DATABASE_URL}" MILVUS_URI="${MILVUS_URI}" &
sleep 2
