...
async function buildAgentMap(): Promise<Record<string, HttpAgent>> {
  ...
  for (const key of FALLBACK_AGENT_KEYS) {
    if (key in agents) {
      continue;
    }
    agents[key] = new HttpAgent({ url: resolveAgUiUrl(key) });
  }

  return agents;
}

export async function handleCopilotKitRequest(request: NextRequest): Promise<Response> {
