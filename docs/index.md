# Documentation Index

## Runtime And Setup

- [Configuration](configuration.md)
- [GCP Setup](gcp_setup.md)
- [Web Runbook](web.md)
- [Typing and Static Analysis](typing.md)
- [Logging Standards](logging.md)
- [Git Identity](git_identity.md)

## Workflow And Operations

- [CI Workflow and PR Status Checks](ci.md)
- [Worktree Multi-Agent Workflow](worktree_multi_agent_workflow.md)
- [Codex Multi-Agent Workflow](codex_multi_agent_workflow.md)
- [Merge Runbook](merge_runbook.md)
- [Feedback Bundles](feedback_bundles.md)
- [Trace Reporting and Search Bundles](trace_reporting_and_search_bundles.md)
- [Subagent Tool Rendering Policy](subagent_tool_rendering_policy.md)

## Tools And Data

- [Agent Tool Bridge](tools/agent_tool_bridge.md)
- [Floor Planner Tool](tools/floor_planner.md)
- [Image Analysis Tools](tools/image_analysis.md)
- [Data Model and Storage](data/index.md)
- [Current Graph Data Access](data/current_graph_data_access.md)
- [Data Patterns](data/data_patterns.md)
- [Runtime DB Migrations](data/migrations.md)

## Archive

- [Documentation Archive](archive/index.md)

## Redirects

- [Image Analysis Tools Redirect](image_analysis_tools.md)

## Legacy

Historical docs/specs/plans/SQL archived under `legacy/` are reference-only.
