...
### 1) Shared typed contracts (`src/tal_maria_ikea/shared/types.py`)
...
### 2) Retrieval service interface (`src/tal_maria_ikea/retrieval/service.py`)
...
- `python -m tal_maria_ikea.ingest.index run --scope germany --strategy v2_metadata_first --subset-limit N --parallelism M`
- `--use-batch` optional, not default.

### 4) Eval CLI interfaces
- Generate eval queries:
  - `python -m tal_maria_ikea.eval.generate --subset-id ... --prompt-version ... --target-count 200`
- Run eval:
  - `python -m tal_maria_ikea.eval.run --index-run-id ... --k 10`
